...
import { CLOSE, DELIVERY_NOTE, PURCHASE_RECEIPT } from '../../constants/app-string';
import { switchMap, map, concatMap, catchError, toArray, bufferCount } from 'rxjs/operators';
import {
  ACCESS_TOKEN,
  AUTHORIZATION,
  BEARER_TOKEN_PREFIX,
  ...
  mapJson(jsonData: { item_name: string; serial_no: string }[]) {
    ...
    _.forEach(jsonData,element => {
      if(out[element.item_name]){
        out[element.item_name].serial_no.push(element.serial_no.toString().toUpperCase())
        return;
      }
      (out[element.item_name] = { serial_no: [element.serial_no.toString().toUpperCase()]});
    });
    return out;
  }
    ...
    return this.salesService.getItemByItemNames(item_names).pipe(
      switchMap((response: any[]) => {
        if (response.length === item_names.length) {
          return (validateFor === PURCHASE_RECEIPT) ?
            ...
            this.validateSerialsWithItem(itemObj, validateFor)
        }
      ...
      catchError(err => {
        if(err && err.error && err.error.message){
          ...
          this.snackBar.open(
            `${err.length}${((validateFor === PURCHASE_RECEIPT) ? '+' : '')} Invalid Serials: ${err
              .splice(0, 5)
              .join(', ')}..`,
            CLOSE,
            { duration: 2500 },
